...
import {
  ...
  TAX_CONFIG_PRODUCT_OPTIONS, addTaxConfigRow, buildTarifDisplay, buildTarifNote,
...
export function TambahKonfigurasiPajakContent() {
  const navigate = useNavigate();
  const { setDirty, guardAction } = useFormGuard();
  const [nama, setNama] = useState("");
  const [kategori, setKategori] = useState<TaxConfigCategory | "">("");
  const [perlakuanHarga, setPerlakuanHarga] = useState<TaxConfigPerlakuanHarga | "">("");
  const [metode, setMetode] = useState<TaxConfigMetode>("Persentase");
  ...
  const hasUnsavedChanges = Boolean(
    nama || kategori || perlakuanHarga || nilaiInput || selectedProducts.length > 0 || tanggalMulai || tanggalBerakhir
  );
  ...
  const handleSubmit = () => {
    if (!isValid || !kategori || !perlakuanHarga) return;
    addTaxConfigRow({
      nama: nama.trim(),
      kategori,
      perlakuanHarga,
      ...
      diterapkanPada: selectedProducts.map(id => TAX_CONFIG_PRODUCT_OPTIONS.find(p => p.id === id)?.label || id),
    ...
    setDirty(false);
    navigate("/superadmin/konfigurasi-pajak", { state: { toast: "Konfigurasi pajak dan biaya berhasil dibuat" } });
  };
      ...
      <div className="flex flex-col gap-6 px-10 pt-8">
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2.5">
            ...
            <p className="text-text-default text-[28px] font-bold leading-8" style={{ fontFamily: "var(--font-body)" }}>Tambah Konfigurasi Baru</p>
          </div>
          <div className="flex items-center gap-2.5 pl-[38px]">
            ...
            <span className="text-text-muted text-sm" style={{ fontFamily: "var(--font-body)" }}>Tambah Konfigurasi Baru</span>
          </div>
        </div>
