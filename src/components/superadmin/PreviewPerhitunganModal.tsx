...
import { TaxConfigMetode } from "../../mocks/konfigurasi-pajak";

function formatRupiah(value: number): string {
  return value.toLocaleString("id-ID");
}

export function PreviewPerhitunganModal({ nama, metode, nilai, onClose }: { nama: string; metode: TaxConfigMetode; nilai: number; onClose: () => void }) {
  const [hargaDasarInput, setHargaDasarInput] = useState("1000000");
  const hargaDasar = Number(hargaDasarInput) || 0;
  const biaya = metode === "Persentase" ? Math.round((hargaDasar * nilai) / 100) : nilai;
  const total = hargaDasar + biaya;
  const configName = nama.trim() || "Nama Konfigurasi";

  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div
        className="bg-white rounded-2xl w-full max-w-[480px] border border-[#e5e9ed] shadow-[0px_8px_32px_0px_rgba(0,0,0,0.12)] px-8 py-7 flex flex-col gap-5"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between w-full">
          <p className="text-[22px] font-bold text-[#212529]" style={{ fontFamily: "var(--font-heading)" }}>Preview Perhitungan</p>
        ...
        <div className="flex flex-col w-full border-t border-b border-[#e5e9ed]">
          ...
          <div className="flex items-center justify-between px-3.5 py-3 text-[14px]">
            <span className="text-text-darker" style={{ fontFamily: "var(--font-body)" }}>{configName}</span>
            <span className="flex items-center gap-1.5">
              {metode === "Persentase" && (
                <span className="text-[#777980]" style={{ fontFamily: "var(--font-body)" }}>({nilai}%)</span>
              )}
              <span className="font-semibold text-text-default" style={{ fontFamily: "var(--font-body)" }}>Rp{formatRupiah(biaya)}</span>
            </span>
          </div>
        </div>
