...
export function CabutBanModal({ nama, onConfirm, onClose }: { nama: string; onConfirm: (reason: string) => void; onClose: () => void }) {
  const [reason, setReason] = useState("");

  return (
    <InfoModal
      title="Cabut Ban Akun?"
      onClose={onClose}
      footer={
    ...
    >
      <div className="flex flex-col gap-4 w-full">
        <p>
          Tindakan ini akan membatalkan sanksi ban pada akun <span className="font-bold text-text-darker">{nama}</span> dan mengembalikan akses akun. Pastikan Anda memiliki dasar yang kuat sebelum melanjutkan.
        </p>
